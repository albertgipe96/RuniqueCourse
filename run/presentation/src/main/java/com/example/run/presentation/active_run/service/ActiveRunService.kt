...
class ActiveRunService : Service() {
    ...
    override fun onBind(p0: Intent?): IBinder? {
        return null // Not needed because service will not communicate with the app
    }
    ...
    override fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
        when (intent?.action) {
            ACTION_START -> {
                val activityClassName = intent.getStringExtra(EXTRA_ACTIVITY_CLASS) ?: throw IllegalArgumentException("No activity class provided")
    ...
    fun stop() {
        ...
        serviceScope = CoroutineScope(Dispatchers.Main + SupervisorJob()) // Needed if we have to relaunch a new scope
    }
